...
const int width = 600;//create constant width&length
...
vec3 draw(const ray &r, hittable *world,int depth) {
   ...
   if(world -> hit(r, 0.001, MAXFLOAT, rec)) {
      ray scat;
      vec3 att;
      if(depth<5 && rec.mat->scatter(r,rec,att,scat)){
...
int main() {
   ...
   /*
   hittable *list[5];
   list[0] = new sphere(vec3(0,0,0), 0.5, new lambertian(vec3(0.1, 0.2, 0.5)));
   list[1] = new sphere(vec3(0,-100.5,0), 100, new lambertian(vec3(0.8, 0.8, 0.0)));
   list[2] = new sphere(vec3(1,0,0), 0.5, new metal(vec3(0.8, 0.6, 0.2), 0.3));
   list[3] = new sphere(vec3(-1,0,0), 0.5, new dielectric(1.5));
   list[4] = new sphere(vec3(-1,0,0), -0.45, new dielectric(1.5));
